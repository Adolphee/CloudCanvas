...
public class CreateThumbnail
{
    ...
    /// <returns>A <see cref="CloudCanvasMessageDTO"/> containing the event details and metadata after the thumbnail creation
        ...
        ServiceBusMessageActions messageActions)
    {
        ...
        BlobMetaDTO metadata = message.Body.ToObjectFromJson<BlobMetaDTO>();
        //BlobMetaDTO metadata = dto.Payload;
        metadata = JsonSerializer.Deserialize<BlobMetaDTO>(message.Body.ToString()); //TODO: wrap in try-catch block
        ...
        try
        {
            ...
            // var bclient = _blobService.GetContainerClientAsync(thumbnails);
            // using var image = await Image.LoadAsync(metadata.BlobUrl); // TOFIX: no overload of this methos takes URLs
            ...
            await messageActions.CompleteMessageAsync(message);  // Complete the message
        }
        ...
        var responseMessage = new ServiceBusMessage(_converter.ToString(body));
