...
using ICosmos = CloudCanvas.Application.Abstractions.Persistence.IPostsRepository<CloudCanvas.Domain.Posts.Contracts.IPost>;
...
public class BlobsApiFunction
{
    ...
    public async Task<IActionResult> Get([HttpTrigger(AuthorizationLevel.Anonymous, "get", Route = "photos")] HttpRequest req)
    {
        try
        {
            ...
            var res = await new GetAllPostsRequestHandler(_cosmos).Handle(new GetAllPhotosQuery());
